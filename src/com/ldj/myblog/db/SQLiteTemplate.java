...
public class SQLiteTemplate {
	...
	public synchronized void insertAll(String table,
			List<ContentValues> contents) {
		try {
	...
	public synchronized void delAllByIds(String table, List<String> ids,
			String idCol) {
		try {
			...
			for (String id : ids) {
				...
				dataBase.delete(table, idCol + " = " + id, null);
			}
	...
	public synchronized void delAllByIds(String table, String[] ids,
			String idCol) {
		try {
	 ...
	 * @return 返回值大于0表示删除成功
	 */
	public synchronized int deleteByField(String table, String field,
			String[] values) {
		try {
			dataBase = dBManager.openDatabase();
			return dataBase.delete(table, field, values);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (!isTransaction) {
	 ...
	 * @return
	 */
	public synchronized <T> T queryForObject(RowMapper<T> rowMapper,
		...
		try {
			...
			// Log.e("==", "data base is closed:" + dataBase.isOpen());
	 ...
	 * @return
	 */
	public synchronized <T> List<T> queryForList(RowMapper<T> rowMapper,
